...
# Version finale avec une fonction de génération de décision robuste.
...
def generate_decision_from_template(template_path, output_path, context):
    ...
    Cette version est plus robuste et gère les tags multiples et les tableaux.
    """
    try:
        ...
        for p in doc.paragraphs:
            # Reconstruire le texte complet du paragraphe une seule fois
            full_text = "".join(run.text for run in p.runs)
            
            # Ne faire le travail que si un tag est potentiellement présent
            if '{{' in full_text and '}}' in full_text:
                
                # Appliquer tous les remplacements sur la chaîne de caractères
                for key, value in context.items():
                    full_text = full_text.replace(key, str(value))
                
                # Vider tous les 'runs' existants et réécrire le contenu final
                # Cela garantit que la mise en forme du premier run (et du paragraphe) est conservée
